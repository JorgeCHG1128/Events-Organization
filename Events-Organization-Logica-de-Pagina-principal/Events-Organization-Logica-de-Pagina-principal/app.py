...
def get_db_connection():
    return pymysql.connect(
        ...
        user='root',
        password='Chac1128',
        database='Analisis',  # Asegúrate de que este sea el nombre correcto de la base de datos
...
@app.route('/field')
def field():
    # Obtener el nombre del evento desde la URL
    name_event = request.args.get('name_event')

    # Aquí puedes hacer una consulta a la base de datos usando `name_event`
    conn = get_db_connection()
    try:
        with conn.cursor() as cursor:
            cursor.execute("SELECT * FROM business.Event WHERE NameEvent = %s", (name_event,))
            event_data = cursor.fetchone()
    except pymysql.Error as e:
        logging.error(f"Error de base de datos: {str(e)}")
        return render_template('error.html', message="Error en la base de datos")
    finally:
        conn.close()

    # Si no se encuentra el evento, redirigir a una página de error o mostrar un mensaje
    if not event_data:
        return render_template('error.html', message="Evento no encontrado")

    # Renderizar la plantilla Field.html y pasar los datos del evento
    return render_template('Field.html', event=event_data)
